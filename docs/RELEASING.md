...
required signing value is absent, or when the resulting binaries do not pass
...
3. Create and push one annotated tag on that exact `main` commit:
...
4. Watch the `Release` workflow. Do not create a GitHub Release by hand. A tag
   containing a SemVer prerelease suffix, such as `v0.2.0-beta.1`, becomes a
   prerelease; a stable tag becomes the latest release.
5. Download one installer per platform and verify its checksum, attestation,
...
artifact and SBOM attestations. The publish job then creates a sorted
`SHA256SUMS.txt` and an immutable GitHub Release.
